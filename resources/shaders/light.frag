...
vec3 calcPointLight(in vec3 diffuseColor, in float specularFactor, in float shininess, in PointLight light, in vec3 position, in vec3 normal, in vec3 camDir)
{
	...
	float attenuationInv = 1 / (light.att.constant + distance * ( light.att.linear + light.att.exponent * distance));
...
vec3 calcFog(in vec3 pos, in vec3 colour)
{
	float factor = length(pos) * fog.density;
    ...
    float fogFactor = 1.0 / exp( factor * factor);
...
void main()
{
	...
	if( normal.a == 0 ) // Non emissive
	{
		...
		vec3 diffuseSpecularComp = calcLightColour(diffuseC, specularFactor, shininess, directionalLight.color, directionalLight.direction, norm, camera_direction) * shadow.x;
		...
		if ( fog.activeFog ) {
			fs_color = calcFog(viewPos, fs_color);
		}
