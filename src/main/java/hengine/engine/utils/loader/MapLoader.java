...
public class MapLoader {
	...
	public static GameItem[] loadMap(final String internalPath) throws Exception {
		...
		while ((line = br.readLine()) != null) {
			if (line.startsWith("nmbItem"))
				...
				tmp.getMesh().getMaterial().setDiffuseColour(new Vector4f(color[0], color[1], color[2], color[3]));
	...
	private static int getInt(final String line, final String tag) {
		return Integer.parseInt(getString(line, tag));
	}

	private static float getFloat(final String line, final String tag) {
		return Float.parseFloat(getString(line, tag));
	}

	private static String getString(final String line, final String tag) {
		final int length = (tag + ":").length();
		return line.substring(length);
	}

	private static float[] getFloats(final String line, final String tag) {
		final String[] part = getString(line, tag).split(" ");
	...
	private static String[] getStrings(final String line, final String tag) {
		return getString(line, tag).split(" ");
	}
	...
	public static GameItem loadInstanced(final String dir, final String extension, final int instances)
			throws Exception {
		final String path = "models/" + dir + "/";
