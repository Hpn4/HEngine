...
public class GameItem implements Comparable<GameItem> {
	...
	public GameItem(final GameItem item) {
		position = new Vector3f(item.position);
		rotation = new Quaternionf(item.rotation);

		modelMatrix = new Matrix4f(item.modelMatrix);
		normalMatrix = new Matrix3f(item.normalMatrix);

		scale = item.scale;
		setMeshes(item.meshes);
	...
	public void updateBox3D() {
		...
		for (int i = 0, cMeshes = meshes.length; i < cMeshes; i++) {
			...
			final Vector3f[] pos = new Vector3f[] { new Vector3f(min), new Vector3f(min.x, min.y, max.z),
					new Vector3f(max.x, min.y, max.z), new Vector3f(max.x, min.y, min.z),

					new Vector3f(min.x, max.y, min.z), new Vector3f(min.x, max.y, max.z), new Vector3f(max),
					new Vector3f(max.x, max.y, min.z) };

			// Pour chacun des point, on le passe en world pose et on extrait le min et ou
			// le max
			for (int j = 0; j < 8; j++) {
	...
	public int compareTo(final GameItem item) {
		return Float.compare(item.viewSpaceDepth, viewSpaceDepth);
	}
}