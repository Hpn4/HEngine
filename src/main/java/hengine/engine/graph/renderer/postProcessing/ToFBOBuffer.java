...
public class ToFBOBuffer {
	...
	public ToFBOBuffer(final HWindow window) throws Exception {
		...
		// Create textures for scene color and emissive
		for (int i = 0; i < TOTAL_TEXTURES; i++) {
			glBindTexture(GL_TEXTURE_2D, ids[i]);

			glTexImage2D(GL_TEXTURE_2D, 0, i == 0 ? GL_RGBA : GL_RGB8, width, height, 0, i == 0 ? GL_RGBA : GL_RGB,
			...
			// Attach the the texture to the framebuffer
