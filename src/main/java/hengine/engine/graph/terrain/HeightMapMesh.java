package hengine.engine.graph.terrain;
...
public class HeightMapMesh {
	...
	public HeightMapMesh(float minY, float maxY, String heightMapFile, String textureFile, final String normalFile,
		...
		final double radius = -STARTX - STARTX;
		double distInc = radius / ((height - 1) / 2);
		double dist = 0;

		final double angleInc = (2 * Math.PI * radius) / (width - 1);
		final double angleMin = 3 * Math.PI / 2; // Le bas du cercle
		...
		for (int row = 0; row < height; row++) {
			for (int col = 0; col < width; col++) {
				...
				final double modX = Math.cos(angle);
				positions.add(STARTX + col * incx + (float) modX); // x

				// Lorsque cos - on ajoute -> décale point vers la droite
				// Lorsque cos + on soustrait -> décale point vers la gauche

				final double modY = Math.sin(angle);
				positions.add(getHeight(col, row, width, buf) + (float) modY); // y
			...
			if (dist >= radius)
				distInc = -distInc; // Pour que la distance se soustrait
		}
		...
		// material.setNormalMap(new Texture(normalFile));
