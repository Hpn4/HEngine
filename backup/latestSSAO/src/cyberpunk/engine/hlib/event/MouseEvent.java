package hengine.engine.hlib.event;
...
public class MouseEvent extends Event {
	...
	public static int BUTTON_LEFT = GLFW.GLFW_MOUSE_BUTTON_LEFT, BUTTON_RIGHT = GLFW.GLFW_MOUSE_BUTTON_RIGHT,
	...
	public MouseEvent(final double x, final double y, final boolean scroll) {
		...
		if (scroll) {
			...
			tmpMousePos.x = (int) Math.round(x * ((float) HWindow.frameWidth / HWindow.initWidth));
			tmpMousePos.y = (int) Math.round(y * ((float) HWindow.frameHeight / HWindow.initHeight));
			mouseOffset = new Vector2i();
		}
