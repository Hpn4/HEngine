...
public class GBuffer {
	...
	public GBuffer(final HWindow window) throws Exception {
		...
		for (int i = 0; i < TOTAL_TEXTURES; i++) {
			...
			if (i == 0) { // Depth
				...
				if(i == 3) // Normal
					...
					internalFormat = GL_RGB8; // Diff/em color, specular and shadow

				glTexImage2D(GL_TEXTURE_2D, 0, internalFormat, width, height, 0, GL_RGB, GL_FLOAT, (ByteBuffer) null);
				// For sampling
			}
		...
		try (final MemoryStack stack = MemoryStack.stackPush()) {
			glDrawBuffers(stack.ints(GL_COLOR_ATTACHMENT0, GL_COLOR_ATTACHMENT1, GL_COLOR_ATTACHMENT2,
